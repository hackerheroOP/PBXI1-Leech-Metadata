...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
    cmd = [bot_cache['pkgs'][2], '-hide_banner', '-loglevel', 'error', '-ignore_unknown', '-i', media_file, '-metadata', f'title={metadata}',
           '-metadata:s:v', f'title={metadata}', '-metadata', 'Comment=', '-metadata', 'Copyright=', '-metadata', f'AUTHOR=『@WeebMatrix』', '-metadata', 'Encoded by=『@WeebMatrix』', '-metadata', 'SYNOPSIS=', '-metadata', 'ARTIST=', '-metadata', 'PURL=', '-metadata', 'Encoded_by=『@WeebMatrix』', '-metadata', 'Description=Join On TG『@WeebMatrix』', '-metadata', 'description=', '-metadata', 'SUMMARY=', '-metadata', 'WEBSITE=', '-metadata:s:a', f'title={metadata}',
